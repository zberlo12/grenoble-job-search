...
Run `cat config.json` via Bash and extract `user.name`, `user.profile_id` → user_profile, `user.base_city`, `background` → functional_expertise, key_systems (for fit assessment).
